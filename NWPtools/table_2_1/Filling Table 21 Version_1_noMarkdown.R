...
table_2.1 <- read.csv(paste(pathIn, table_2.1_ref, sep='/'), sep='
', header=TRUE, as.is=TRUE, encoding='UTF-8') #New
...
if (whatGoesIn$EUROSTAT) {tabControl$EURO.CTRY=NA
 tabControl$EURO.EU=NA}
if (whatGoesIn$FIDES)    {tabControl$FIDES.CTRY=NA
  tabControl$FIDES.EU = NA
 tabControl$FIDES.SHARE=NA}
if (whatGoesIn$NationalStats) tabControl$NAT.CTRY=NA
if (whatGoesIn$RCGstats) {tabControl$RCG.CTRY=NA
 tabControl$RCG.EU=NA}
...
# -----------------------------------------------------------------------
 ...
 T1$sourceEU <- sourceTAC[paste0(isFides)]}
		T1$comments<-NA
...
write.table(tabControl, file=paste(pathOut,paste(CTRY,_table2_1_control, output_suffix, .csv,sep=''),sep='/'), sep='
